	...
	public:
		...
		void drawPoint(Graphics ^g, Point ^start, Point ^end, ViewInfo ^info, Pen ^pen) override {
			for (float st = 0; st <= Math::PI * 2; st += Math::PI / 1200) {
				String ^expr = this->FuncString->Replace("θ", st.ToString());
				float p = calc(expr);
				FPoint ^point1 = transform(p, st);
				st += Math::PI / 1200;
				expr = this->FuncString->Replace("θ", st.ToString());
				p = calc(expr);
		...
		FPoint ^transform(float ρ, float θ) {
			return gcnew FPoint(ρ * Math::Cos(θ), ρ * Math::Sin(θ));
		}
	};
